...
export default function SignupPage() {
      ...
      if (e.key === "Enter" && !e.shiftKey) {
        ...
        if (currentStep < 4) {
          ...
          if (currentStep === 1) {
            ...
            isCurrentStepValid = name.trim().length > 0 && 
                   name === name.trim() &&
  ...
  }, [currentStep, name, id, password, confirm, birth, address, isIdChecked, idCheckStatus, lastCheckedId]);
  ...
  }, [name, id, isIdChecked, idCheckStatus, lastCheckedId]);
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              <div className="text-center mb-4 sm:mb-6">
                ...
                <p className="text-xs sm:text-sm text-gray-600">이름과 아이디를 입력해주세요</p>
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="이름(실명)"
                  className={`h-12 text-base ${getInputStatus(name, name.trim().length > 0 && /^[a-zA-Z가-힣]+$/.test(name.trim()), name && name !== name.trim())}`}
                  aria-label="이름 입력"
                  aria-describedby={name && name !== name.trim() ? "name-error" : undefined}
                />
                {name && name !== name.trim() && (
                  <p id="name-error" className="text-xs text-red-600 mt-1" role="alert">앞뒤 공백을 제거해주세요</p>
                )}
                {name.trim() && /^[a-zA-Z가-힣]+$/.test(name.trim()) && (
