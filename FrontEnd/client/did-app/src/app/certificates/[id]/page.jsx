...
export default function CertificateDetailPage({ params }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      setCertificate(JSON.parse(saved));
    } else {
      // 세션스토리지에 데이터가 없으면 목록으로 돌아가기
  ...
  const handleShare = async () => {
    ...
    const certificateName = certificate.title; // 인코딩하지 않은 원본 제목
    
    // 팀원 요청 방식: /did/공개키/certificateName
...
// 수료증 상세 페이지의 handleRevoke 함수만 수정

// handleRevoke 함수에 디버깅 추가
